...
void convert_greyscale(unsigned char* rawimage, int startHeight, int width, int stopHeight) {
	for (int row = startHeight; row < stopHeight; row++) {
		for (int col = 0; col < width; col++) {
			unsigned int blå = *(rawimage + gPixelStart + (3 * col) + (3 * width * row));
			unsigned int grön = *(rawimage + gPixelStart + (3 * col) + (3 * width * row) + 1);
			unsigned int röd = *(rawimage + gPixelStart + (3 * col) + (3 * width * row) + 2);
